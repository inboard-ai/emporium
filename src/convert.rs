...
//! ...) generates DISTINCT Rust types for shared interfaces like
//! `tool-provider` and `host-events`, so the From/TryFrom impls are
//! duplicated here by hand — one set per bindings module.

use emporium_core::{block, column, event, plan, tool};

use crate::Error;
use crate::wasm::{block_bindings, tool_bindings};
