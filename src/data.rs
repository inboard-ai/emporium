...
impl ToolResult {
    ...
    pub fn text<T: Into<String>>(text: T) -> Result<Self, Error> {
        Ok(ToolResult::Text(text.into()))
    }

    /// Create a new DataFrame result
    pub fn columnar(data: serde_json::Value, column_defs: Schema) -> Result<Self, Error> {
        ...
        let arr = data
            .as_array()
            .ok_or_else(|| Error::Custom("Data must be a JSON array".to_string()))?;
        ...
        let dataframe =
            DataFrame::new(columns_vec).map_err(|e| Error::Custom(format!("Failed to create DataFrame: {}", e)))?;
