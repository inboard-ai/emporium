...
fn run_tests(tx: &futures::channel::mpsc::UnboundedSender<Message>) {
    let send = |cmd: Value| {
        let msg_str = cmd.to_string();
        // eprintln!("Sending message: {}", msg_str);
        tx.unbounded_send(Message(msg_str)).unwrap();
    };
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some((path, manifest)) = list.next().await {
        if manifest.id == "kv" {
            kv_extension = Some(emporium::load(manifest.id, path).await?);
            break;
        }
    }
